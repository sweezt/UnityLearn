...
namespace VRTK
{
    ...
    public class VRTK_SnapDropZone : MonoBehaviour
    {
        ...
        protected virtual VRTK_InteractableObject ValidSnapObject(GameObject checkObject, bool grabState, bool checkGrabState = true)
        {
            ...
            if (ioCheck == null)
                return null;
            var grabed = ioCheck.IsGrabbed();
            if (checkGrabState && grabed != grabState)
                return null;
            var pcheck = VRTK_PolicyList.Check(ioCheck.gameObject, validObjectListPolicy);
            return pcheck ? null : ioCheck;
