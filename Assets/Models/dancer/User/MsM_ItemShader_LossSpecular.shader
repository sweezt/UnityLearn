...
Shader "Toony Colors Pro 2/User/MsM_ItemShader_LossSpecular"
{
	Properties
	{
	...
	[TCP2HeaderHelp(TRANSPARENCY)]
		...
		_CameraDistance("Camera Distance", Float) = 30
	...
	SubShader
	{
		...
		#pragma surface surf ToonyColorsCustom nofog fullforwardshadows halfasview addshadow vertex:vert//noforwardadd 
		#pragma target 3.0
		...
		inline half4 LightingToonyColorsCustom (inout SurfaceOutputCustom s,half3 viewDir, UnityGI gi)
		{
			...
			fixed4 c;
			spec *= s.CameraDist;
			c.rgb = s.Albedo * gi.light.color * ramp;

			c.rgb += gi.light.color * _SpecColor.rgb * spec;
		...
		void vert(inout appdata_full v,out Input o)
		{
			...
			float3 view = ObjSpaceViewDir(v.vertex);
  			float d = dot(view,view);			
  			float x = max(1.0,d - _CameraDistance);
  			o.cameraDist = 1.0 / x;
		}

		void surf(Input IN, inout SurfaceOutputCustom o)
		{
			fixed4 mainTex = tex2D(_MainTex, IN.mainTexUV);
			
			//Masks
			fixed4 mask1 = tex2D(_Mask1, IN.mainTexUV);
			...
			o.CameraDist = IN.cameraDist;
			//Normal map
			half4 normalMap = tex2D(_BumpMap, IN.bumpMapUV.xy);
